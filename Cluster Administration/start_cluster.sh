...
gcutil addinstance master-node --disk=master-node-disk-2,boot --machine_type=n1-highmem-2 --zone=europe-west1-b > /dev/null 2>&1 &
...
gcutil addinstance slave-d-node --disk=slave-d-node-disk,boot --machine_type=n1-standard-1 --zone=europe-west1-b > /dev/null 2>&1 &
gcutil addinstance slave-e-node --disk=slave-e-node-disk,boot --machine_type=n1-standard-1 --zone=europe-west1-b > /dev/null 2>&1
sleep 60
...
gcutil ssh slave-d-node "sudo reboot" > /dev/null 2>&1 &
gcutil ssh slave-e-node "sudo reboot" > /dev/null 2>&1
sleep 60
echo
echo "Nodes started"
